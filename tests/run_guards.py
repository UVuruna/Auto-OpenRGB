...
never the full behavioral suite (`test_color_groups.py`, `test_imports.py`,
`test_schedule.py`, `test_tasks.py` belong to `python -m pytest tests/`, not
here).
...
mode, a call for a file that is NOT `.py` exits 0 immediately, before
...
def _fast_exit_for_non_source() -> bool:
    """True when this is a PostToolUse call for a file that is not .py —
